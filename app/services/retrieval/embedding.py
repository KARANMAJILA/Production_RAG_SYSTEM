...
def _probe_gemini():
    ...
    try:
        model = GoogleGenerativeAIEmbeddings(
            ...
            google_api_key=settings.GOOGLE_GEMINI_KEY,
        )
        model.embed_query("probe")
        logfire.info("Gemini embeddings ready (gemini-embedding-2-preview, 3072-dim).")
        return model
    except Exception as e:
        logfire.warning(f"Gemini probe failed: {e!r}. Will use sentence-transformers fallback.")
        return None
...
def _init(force: bool = False):
    ...
    Set force=True (or call reset_embedding_model() beforehand) to re-probe
    Gemini — useful if you fixed configuration (e.g. an API key) without
    restarting the process, since otherwise the cached fallback model would
    be reused forever.
    """
    global _active_model, _model_type
    if _active_model is not None and not force:
        return
...
def reset_embedding_model():
    """Clear the cached model so the next call re-probes Gemini from scratch.

    Call this after fixing Gemini configuration at runtime if you don't want
    to restart the process. Safe to call at any time; it's just a cache clear.
    """
    global _active_model, _model_type
    _active_model = None
    _model_type = None
...
def get_active_model_type() -> str:
    """Return which model is currently active: 'gemini' or 'fallback'.

    Handy for a health-check endpoint or for debugging silent fallbacks.
    """
    _init()
    return _model_type
